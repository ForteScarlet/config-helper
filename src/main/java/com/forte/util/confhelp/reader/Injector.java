...
import com.forte.util.confhelp.ConfigsHelper;
...
public class Injector<T> {
     ...
     * TODO 测试
     */
    public Injector(Prop.PropData[] propDatas, Class<T> type){
...
//                        //如果值为null，直接注入setter
//                        try {
//                            fieldSetter.invoke(b, (Object) null);
                ...
                if(value.getClass().isArray() || FieldUtils.isChild(value, List.class)){
                    ...
                    String valueString = FieldUtils.isChild(value, String.class) ? (String) value : String.valueOf(value);
                        ...
                        if(String[].class.equals(needs)){
                            ...
                            //转化类型并返回
                            return (N) Arrays.stream(valueSplit).map(v -> ConvertUtils.convert(value, listFieldGeneric)).collect(Collectors.toList());
                        }else{
                            //否则，是类型不同的数组类型，获取数组的类型
                            Class arrayGeneric = FieldUtils.getArrayGeneric(needs);
                            //尝试转化并返回
                            return (N) Arrays.stream(valueSplit).map(v -> ConvertUtils.convert(value, arrayGeneric)).toArray();
                        }
                }
            }else {
